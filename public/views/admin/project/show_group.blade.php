...
@section('content')
    ...
    <div class="am-u-sm-10">
        <form class="am-form am-form-horizontal" action="/project/update/{{ $group->id }}" method="post">
            ...
            <div class="am-form-group">
                ...
                <div class="am-u-sm-10">
                    <textarea name="remark" rows="5" id="doc-ta-1">{{ $group->remark }}</textarea>
                </div>
            </div>
            ...
            <div class="am-form-group">
                ...
                <div class="am-u-sm-10">
                    <select multiple data-am-selected name="type">
                        <option value="1" {{ $group->type==1?"selected":"" }}>测试环境</option>
                        <option value="2" {{ $group->type==2?"selected":"" }}>预发布环境</option>
                        <option value="3" {{ $group->type==3?"selected":"" }}>线上环境</option>
                    </select>
                </div>
            </div>
                ...
                <div class="am-u-sm-10">
                    <label class="am-radio-inline">
                        <input type="radio" name="status" value="1"
                               {{ $group->status?'checked="checked"':'' }} data-am-ucheck> 启用
                    </label>
                    <label class="am-radio-inline">
                        <input type="radio" name="status" value="0"
                               {{ !$group->status?'checked="checked"':'' }} data-am-ucheck> 停用
                    </label>
                </div>
            </div>
                    ...
                    @foreach($envGroup as $groupName=>$envs)
                        @foreach($envs as $env)
                            <label class="am-checkbox am-secondary">
                                <input type="checkbox" name="hosts[]" value="{{ $env->id }}" data-am-ucheck {{ $env->checked?'checked':""  }}>
                                {{ $env->title }} : {{ $env->host }}
                            </label>
                        @endforeach
                    @endforeach
